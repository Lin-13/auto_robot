...
int main(int argc, char** argv) {
    ...
    //demo.match_img_loader_.Init(demo.left_calib_imgs_, demo.right_calib_imgs_);//use calib imgs
...
int main_fcn() try{
    ...
    std::chrono::system_clock::rep check_point_time_rep = check_point_time.time_since_epoch().count();
    // rep:long long for system_clock
    std::cout << "System time for checkpoints: " << check_point_time_rep << std::endl;
    std::string checkpoints_main_path = std::string("checkpoints/") + std::to_string(check_point_time_rep);
    ...
    //Calibration method
    Calib(imgs1, boardWidth, boardHeight, squareSize, 
        cameraMatrix1, distCoeffs1, objPointsList, img1PointList, chess1);
    objPointsList.clear();
    Calib(imgs2, boardWidth, boardHeight, squareSize, 
        cameraMatrix2, distCoeffs2, objPointsList, img2PointList, chess2);
    ...
    cv::FileStorage file(checkpoints_main_path + "/param.xml",cv::FileStorage::WRITE);
    file << "Q" << Q;
