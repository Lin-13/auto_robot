...
void CameraParams::Load(std::string map_xml, std::string matrix_xml) {
    ...
    if (!map.isOpened()) {
        throw std::string("Map can not load from xml file.\n") +
            "Better code use xml to save CV_32FC1 Matrix,instead of jpg,in the old version."
            "If file: " +
            map_xml +
            " does not exist,you can load stereo imgs in this point" +
...
int StereoImageLoader::Init(std::vector<cv::Mat>& can1, std::vector<cv::Mat>& can2) {
    if (can1.size() != can2.size()) {
        throw "StereoImageLoader::StereoImageLoader::Containers must have the same size";
    }
    type_ = MAT_CONTAINER;
    container_idx_ = 0;
    container_max_ = -1;
...
CalibChessBoard::CalibChessBoard() {
    ...
    checkpoints_main_path_ = std::string("checkpoints/") + std::to_string(checkpoints_time_rep_);
...
void CalibChessBoard::LoadImgsForCalib(long long checkpoints, int num) {
    std::string main_path = checkpoints == 0 ?
        checkpoints_main_path_ :
        std::string("checkpoints/") + std::to_string(checkpoints);
    imgsLoad(left_calib_imgs_, main_path + "/stereo/left", num);
    imgsLoad(right_calib_imgs_, main_path + "/stereo/right", num);
}
...
void CalibChessBoard::LoadParamsCheckPoints(std::string check_points_main_path) {
...
void CalibChessBoard::LoadParamsCheckPoints(long long checkpoints) {
    LoadParamsCheckPoints(
        checkpoints == 0 ?
        checkpoints_main_path_ :
        std::string("checkpoints/") + std::to_string(checkpoints));
}
void CalibChessBoard::SaveParamsCheckPoints(std::string check_points_main_path) {
...
void CalibChessBoard::CalibStart(int boardWidth,int boardHeight,float squareSize) {
    //Calibration start!
    ...
    Calib(left_calib_imgs_, boardWidth, boardHeight, squareSize,
        params_.cameraMatrix1, params_.distCoeffs1, objPointsList, img1PointList, chess1);
    objPointsList.clear();
    //Calib right camera
    Calib(right_calib_imgs_, boardWidth, boardHeight, squareSize,
        params_.cameraMatrix2, params_.distCoeffs2, objPointsList, img2PointList, chess2);
    ...
    stereoCalibrateAndRectify(objPointsList, img1PointList, img2PointList,
        params_.cameraMatrix1, params_.distCoeffs1, params_.cameraMatrix2, params_.distCoeffs2,
        imageSize,
        params_.mapX1, params_.mapY1, params_.mapX2, params_.mapY2,
        params_.Q);
}
void CalibChessBoard::Match() {
    ...
    while (1) {
        ...
        //std::cout << "Time for remap: " << (end-start).count() / 1e9 << " s." << std::endl;
        ...
        //std::cout << "Time for match: " << (end - start).count() / 1e9 << " s." << std::endl;
        //dis.convertTo(disparity, CV_32F);

        //Disparity scaling (disparity.type() == CV_16SC1)==true
        //std::cout << "\nDisparity type:" << (disparity.type() == CV_16SC1) << std::endl;
        //bias=0
        disparity.convertTo(disparity_norm, CV_8UC1, 
            255.0 / (scale*16.0), bias * 255.0 / (scale * 16.0));
        ...
        pointcloud *= 16;
        //std::cout << "\nPointcloud type:"<< (pointcloud.type() == CV_32FC3) << std::endl;
        cv::split(pointcloud, pointxyz);
        pointcloudZ = pointxyz[2];
        //PointcloudProcess(pointcloud, 10000);
        cv::absdiff(pointcloudZ, cv::Mat::zeros(pointcloud.size(), CV_32FC1), pointcloudZ);
        //Output and show
...
int readImageFile(const std::vector<std::string> imgFile, std::vector<cv::Mat>& imgs) {
    bool success = true;
...
//Calib Mono
int Calib(std::vector<cv::Mat> imgs,
    int boardWidth,
    int boardHeight,
    float squareSize,
    cv::Mat& cameraMatrix,
    ...
    std::vector<int>& find_chessboard,
    bool output)
    try {
    ...
    cv::calibrateCamera(calib_obj, calib_img, gray.size(),
...
int imgsFilter(std::vector<cv::Mat> imgs1, std::vector<int> find1,
    ...
    std::vector<cv::Mat>& new_imgs1, std::vector<cv::Mat>& new_imgs2) {
    new_imgs1.clear();
    new_imgs2.clear();
...
int PointListFilter(std::vector<std::vector<cv::Point3f>>& objPointsList,
    ...
    if (img1PointList.size() != img1PointList.size() ||
        ...
        img1PointList.size() != flags1.size() ||
        flags1.size() != flags2.size()) {
...
int stereoCalibrateAndRectify(std::vector<std::vector<cv::Point3f>> objPointsList,
    std::vector<std::vector<cv::Point2f>> img1PointList,
    std::vector<std::vector<cv::Point2f>> img2PointList,
    cv::Mat cameraMatrix1, cv::Mat distCoeffs1, cv::Mat cameraMatrix2, cv::Mat distCoeffs2,
    cv::Size imageSize,
    cv::Mat& mapX1, cv::Mat& mapY1, cv::Mat& mapX2, cv::Mat& mapY2, cv::Mat& Q
)try {
    cv::Mat R, T, E, F;
    ...
    double reprojectionError = cv::stereoCalibrate(objPointsList, img1PointList, img2PointList,
    ...
    cv::stereoRectify(cameraMatrix1, distCoeffs1, cameraMatrix2, distCoeffs2, imageSize,
        R, T, R1, R2, P1, P2, Q, 1024, 0, newSize, &roi1, &roi2);
    //flags=-1:default;flags=0:fill vaild image
    std::cout << "StereoRectify: newsize " << newSize <<
        " roi1:" << roi1 <<
        " roi2:" << roi2 << std::endl;
    //cv::Mat mapX1, mapY1, mapX2, mapY2;
    cv::Size newImageSize1, newImageSize2;
    cv::Rect ValidROI1, validROI2;
    cv::Mat newCameraMatrix1 = cv::getOptimalNewCameraMatrix(cameraMatrix1, distCoeffs1, imageSize, 1);
    cv::Mat newCameraMatrix2 = cv::getOptimalNewCameraMatrix(cameraMatrix2, distCoeffs2, imageSize, 1);
...
void DisparityColoring(cv::Mat& dis_norm, cv::Mat& colored) {
    ...
    dis8u = dis_norm;
    if (colored.empty() || colored.type() != CV_8UC3 || colored.size() != dis_norm.size()) {
...
void PointcloudProcess(cv::Mat& pointcloud,float threshold) {
    ...
    for (auto& point : pointcloudxyz) {
        ...
        auto end = std::chrono::steady_clock::now();
        std::cout << "Time of inpaint: " 
            << (end - start).count() / 1e9 << " s." << std::endl;
