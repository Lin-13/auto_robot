...
static std::string getStringInXml(const std::string &xml_s,
...
template <typename T>
static bool getArrayFromString(const std::string &str, const char delim,
                               T *data, size_t len) {
...
template <typename T>
static bool getArrayFromXml(const std::string &xml_s, const std::string &tag,
                            const char delim, T *data, size_t len) {
...
FTSensor::~FTSensor() {
  stopStreaming();
  if (!closeSockets())
    std::cerr << message_header() << "Sensor did not shutdown correctly"
              << std::endl;
  delete setbias_;
}
...
bool FTSensor::startStreaming(int nb_samples) {
  if (nb_samples < 0) {
    ...
    default:
      std::cout << message_header() << cmd_.command
                << ": command mode not allowed" << std::endl;
      return false;
    }
  }
}
...
bool FTSensor::init(std::string ip, int calibration_index, uint16_t cmd,
  ...
  if (!ip.empty() && openSockets()) {
    if (!stopStreaming()) // if previously launched
      std::cerr << "\033[33m" << message_header()
                << "Could not stop streaming\033[0m" << std::endl;
    setCommand(cmd); // Setting cmd mode

    initialized_ &= startStreaming(sample_count); // Starting streaming

    if (!initialized_) {
      std::cerr << "\033[1;31m" << message_header()
                << "Could not start streaming\033[0m" << std::endl;
      return initialized_;
    }
  ...
  if (!initialized_)
    std::cerr << "\033[1;31m" << message_header()
              << "Error during initialization, FT sensor NOT started\033[0m"
              << std::endl;
...
bool FTSensor::openSockets() {
  try {
    ...
    std::cerr << "\033[1;31m" << message_header()
              << "openSockets error: " << ex.what() << "\033[0m" << std::endl;
    return false;
  }
  return true;
}
void FTSensor::openSocket(int &handle, const std::string ip,
                          const uint16_t port, const int option) {
  // create the socket
  if (handle != -1)
    rt_dev_close(handle);
  std::string proto = "";
  ...
  if (handle < 0) {
    std::cerr << "\033[31m" << message_header()
              << "Could not init sensor socket for proto [" << proto
              << "], please make sure your can ping the sensor\033[0m"
              << std::endl;
  ...
  if (rt_dev_connect(handle, (struct sockaddr *)&addr, sizeof(addr)) < 0) {
    std::cerr << "\033[31m" << message_header() << "Could not connect to " << ip
              << "\033[0m" << std::endl;
...
bool FTSensor::getCalibrationData() {
  ...
  if (err != SETTINGS_REQUEST_ERROR && err != CALIB_PARSE_ERROR) {
    std::cout << message_header()
              << "Sucessfully retrieved counts per force : " << resp_.cpf
              << std::endl;
    std::cout << message_header()
...
FTSensor::settings_error_t FTSensor::getSettings() {
  ...
  if (calibration_index != ati::current_calibration) {
    ...
    index = "?index=" + ss.str();
    std::cout << message_header() << "Using calibration index "
              << calibration_index << std::endl;
  } else
    std::cout << message_header() << "Using current calibration" << std::endl;

  static const uint32_t chunkSize =
      4; // Every chunk of data will be of this size
  static const uint32_t maxSize = 65536; // The maximum file size to receive
                                         // The recv buffer
  std::string filename =
      "/netftapi2.xml" + index; // the name of the file to reveice
  std::string host = getIP();

  std::string request_s =
      "GET " + filename + " HTTP/1.1\r\nHost: " + host + "\r\n\r\n";

  if (rt_dev_send(socketHTTPHandle_, request_s.c_str(), request_s.length(), 0) <
      0) {
    std::cerr << message_header() << "Could not send GET request to " << getIP()
              << ":80. Please make sure that RTnet TCP protocol is installed"
              << std::endl;
  ...
  std::cerr << message_header() << "Could not parse file " << filename
            << std::endl;
  return SETTINGS_REQUEST_ERROR;
}
...
bool FTSensor::sendTCPrequest(std::string &request_cmd) {
  if (request_cmd.empty()) {
    std::cerr << message_header() << "Empty TCP command, not sending"
              << std::endl;
    return false;
  } else {
    static const uint32_t chunkSize =
    ...
    if (rt_dev_send(socketHTTPHandle_, request_s.c_str(), request_s.length(),
                    0) < 0) {
    ...
    {
      recvLength =
          rt_dev_recv(socketHTTPHandle_, &xml_c_[posBuff], chunkSize, 0);
      posBuff += recvLength;
      if (recvLength <= 0) // The last chunk returns 0
        break;
    }
    if (posBuff > 4) {
      ...
      if (strncmp(xml_c_, awaited_response, 18) == 0) {
        ...
        std::cerr << message_header()
                  << "Bad response from set command. Response is :" << xml_c_
                  << std::endl;
        return false;
      }
    } else {
...
bool FTSensor::setRDTOutputRate(unsigned int rate) {
  if (rate > 0 && rate <= 7000) {
    ...
    std::cerr << message_header() << "RDT rate must be in range [1-7000]"
              << std::endl;
    return false;
  }
}
...
std::vector<int> FTSensor::getGaugeBias() {
  ...
  if (err != SETTINGS_REQUEST_ERROR && err != GAUGE_PARSE_ERROR) {
    ...
    std::cerr << message_header() << "Could not get gauge bias values"
              << std::endl;
...
bool FTSensor::setGaugeBias(std::map<unsigned int, int> &gauge_map) {
  ...
  for (it = gauge_map.begin(); it != gauge_map.end(); ++it) {
    if (it->first < 6) {
      ...
      std::cerr << message_header() << "Invalid gauge number " << it->first
                << std::endl;
      return false;
    }
  }
...
bool FTSensor::sendCommand() { return sendCommand(cmd_.command); }

bool FTSensor::sendCommand(uint16_t cmd) {
  *reinterpret_cast<uint16_t *>(&request_[0]) =
      htons(command_s::command_header);
  *reinterpret_cast<uint16_t *>(&request_[2]) = htons(cmd);
  *reinterpret_cast<uint32_t *>(&request_[4]) = htonl(cmd_.sample_count);
  // return rt_dev_sendto(socketHandle_, (void*) &request_, sizeof(request_), 0,
  // (sockaddr*) &addr_, addr_len_ ) == 8;
  return rt_dev_send(socketHandle_, (void *)&request_, sizeof(request_), 0) ==
         sizeof(request_); //, (sockaddr*) &addr_, addr_len_ ) == 8;
}

bool FTSensor::getResponse() {

  // response_ret_ = rt_dev_recvfrom(socketHandle_, (void*) &response_,
  // sizeof(response_), 0, (sockaddr*) &addr_, &addr_len_ );
  response_ret_ =
      rt_dev_recv(socketHandle_, (void *)&response_, sizeof(response_),
                  0); //, (sockaddr*) &addr_, &addr_len_ );
  ...
  resp_.Fx = static_cast<int32_t>(
      ntohl(*reinterpret_cast<int32_t *>(&response_[12 + 0 * 4])));
  resp_.Fy = static_cast<int32_t>(
      ntohl(*reinterpret_cast<int32_t *>(&response_[12 + 1 * 4])));
  resp_.Fz = static_cast<int32_t>(
      ntohl(*reinterpret_cast<int32_t *>(&response_[12 + 2 * 4])));
  resp_.Tx = static_cast<int32_t>(
      ntohl(*reinterpret_cast<int32_t *>(&response_[12 + 3 * 4])));
  resp_.Ty = static_cast<int32_t>(
      ntohl(*reinterpret_cast<int32_t *>(&response_[12 + 4 * 4])));
  resp_.Tz = static_cast<int32_t>(
      ntohl(*reinterpret_cast<int32_t *>(&response_[12 + 5 * 4])));
  if (response_ret_ < 0) {
    std::cerr << "\033[1;31m" << message_header()
              << "Error while receiving: " << strerror(errno) << "\033[0m"
              << std::endl;
  }
  if (response_ret_ != RDT_RECORD_SIZE)
    std::cerr << message_header() << "Error of package size " << response_ret_
              << " but should be " << RDT_RECORD_SIZE << std::endl;
...
void FTSensor::doComm() {
  if (isInitialized()) {
    if (cmd_.sample_count != 0) // do not repeat send if infinite samples
      if (!sendCommand())
        std::cerr << message_header() << "Error while sending command"
                  << std::endl;
    if (!getResponse())
      std::cerr << message_header()
                << "Error while getting response, command:" << cmd_.command
                << std::endl;
  }
}
...
bool FTSensor::isInitialized() { return initialized_; }
...
void FTSensor::setTimeout(float sec) {
  ...
  if (isInitialized()) {
    std::cerr << message_header()
              << "Can't set timeout if socket is initialized, call this before "
                 "init()."
              << std::endl;
    return;
  }
...
bool FTSensor::resetThresholdLatch() {
  if (!sendCommand(command_s::RESET_THRESHOLD_LATCH)) {
    std::cerr << message_header() << "Could not start reset threshold latch"
              << std::endl;
    return false;
  }
  return true;
...
bool FTSensor::stopStreaming() { return sendCommand(command_s::STOP); }

bool FTSensor::startBufferedStreaming(uint32_t sample_count) {
  setSampleCount(sample_count);
  setCommand(command_s::BUFFERED);
  if (!sendCommand()) {
    std::cerr << message_header() << "Could not start buffered streaming"
              << std::endl;
    return false;
  }
  return true;
...
bool FTSensor::startMultiUnitStreaming(uint32_t sample_count) {
  ...
  if (!sendCommand()) {
    std::cerr << message_header() << "Could not start multi-unit streaming"
              << std::endl;
    return false;
  }
  return true;
...
bool FTSensor::startRealTimeStreaming(uint32_t sample_count) {
  ...
  if (!sendCommand()) {
    std::cerr << message_header() << "Could not start realtime streaming"
              << std::endl;
    return false;
  }
  std::cout << message_header() << "Start realtime streaming with "
            << sample_count << " samples " << std::endl;
  return true;
}

void FTSensor::setCommand(uint16_t cmd) { this->cmd_.command = cmd; }
