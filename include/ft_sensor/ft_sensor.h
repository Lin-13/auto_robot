...
namespace ati {
static const std::string default_ip = "192.168.1.112";
...
public:
  ...
  // 读取真实数据值
  template <typename T> void getMeasurements(T measurements[6]) {
    doComm();
...
protected:
  ...
  bool openSockets();
  void openSocket(int &handle, const std::string ip, const uint16_t port,
                  const int option);
  bool closeSockets();
